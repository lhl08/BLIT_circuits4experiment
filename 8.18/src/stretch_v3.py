...
class SimpleMaker:
    ...
    def makeSimple(self, path, wire_length, reverse=False):
        ...
        if self.size > 0:
            #宽度为40单位对应1cm            
            lastPoint = array([60,176])
            path.M(lastPoint[0], lastPoint[1])
            for i in range(0,5):
                path.L(lastPoint[0] - 6, lastPoint[1] - 9)
                lastPoint[0] -= 6
                lastPoint[1] -= 9
                path.L(lastPoint[0] - 12, lastPoint[1] + 18)
                lastPoint[0] -= 12
                lastPoint[1] += 18
                path.L(lastPoint[0] - 6, lastPoint[1] - 9)
                lastPoint[0] -= 6
                lastPoint[1] -=9
            path.L(-60, 120)
            lastPoint = array([-60, 120])
            #电阻回弯三次
            for i in range(0,3):
                path.L(lastPoint[0], lastPoint[1] - 280)
                lastPoint[1] = lastPoint[1] - 280
                path.L(lastPoint[0] - 8, lastPoint[1] + 4)

                #打结处
                lastPoint[0] = lastPoint[0] - 8
                lastPoint[1] = lastPoint[1] + 4
                for j in range(0,4):
                    path.L(lastPoint[0] + 16, lastPoint[1] + 4)
                ...
                path.L(lastPoint[0] + 40, lastPoint[1] + 240)
                lastPoint[0] += 40
                lastPoint[1] += 240
            ...
            path.L(lastPoint[0], lastPoint[1] - 240)
            lastPoint[1] -= 240
            
            path.L(lastPoint[0], lastPoint[1] - 56)
            lastPoint[1] -= 56
            path.L(lastPoint[0] - 120, lastPoint[1])
            lastPoint[0] -= 120
...
if __name__=="__main__":
    SimpleMaker.GenerateSimple("narrow_stretch", -8, 10)