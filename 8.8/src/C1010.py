...
class CapacitorMaker:
    ...
    def makeInterdigitalCapacitor(self, path, wire_length, reverse = False):
        ...
        if self.size > 0:
            ...
            path.L(-95,lastPoint[1] - finger_length/2 + spacing[1])

            path.L(-95, lastPoint[1] - finger_length/2 + spacing[1]-5)
            path.L(-90, lastPoint[1] - finger_length/2 + spacing[1]-5)
            path.L(-90, lastPoint[1] - finger_length/2 + spacing[1])
        else:
...
if __name__=="__main__":
    CapacitorMaker.GenerateInterdigitalCapacitor("c1010", 2, 100, 50)
