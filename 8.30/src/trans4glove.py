...
class SimpleMaker:
    ...
    def makeSimple(self, path, wire_length, reverse=False):
        ...
        if self.size > 0:
            #40单位对应1cm
            startPoint = array([-100, -100])
            lastPoint = array([0, 0])
            lastPoint[0] = startPoint[0]
            lastPoint[1] = startPoint[1]
            path.M(lastPoint[0], lastPoint[1])
            for i in range(0,3):
                path.L(lastPoint[0], lastPoint[1] + 10)
                path.L(lastPoint[0] - 10, lastPoint[1] + 10)
                path.L(lastPoint[0] - 10, lastPoint[1])
                path.L(lastPoint[0], lastPoint[1])
                if i < 2:
                    path.L(lastPoint[0] + 80, lastPoint[1])
                    lastPoint[0] += 80
            path.L(lastPoint[0], lastPoint[1] + 160)
            lastPoint[1] += 160
            path.L(lastPoint[0] - 10, lastPoint[1])
            path.L(lastPoint[0] - 10, lastPoint[1] - 10)
            path.L(lastPoint[0], lastPoint[1] - 10)
            ...
            for i in range(0,3):
                ...
                if i < 2:
                    path.L(lastPoint[0] + 80, lastPoint[1])
                    lastPoint[0] += 80
            path.L(lastPoint[0] + 40, lastPoint[1])
            lastPoint[0] += 40
            path.L(lastPoint[0], lastPoint[1] + 200)
            lastPoint[1] += 200
            path.L(lastPoint[0] + 10, lastPoint[1])
            path.L(lastPoint[0] + 10, lastPoint[1] - 10)
            path.L(lastPoint[0], lastPoint[1] - 10)
...
if __name__=="__main__":
    SimpleMaker.GenerateSimple("trans4glove", -8, 10)
