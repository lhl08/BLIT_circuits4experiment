...
class SimpleMaker:
    ...
    def makeSimple(self, path, wire_length, reverse=False):
        ...
        if self.size > 0:
            ...
            lastPoint[1] = startPoint[1]
            path.M(lastPoint[0], lastPoint[1])
            print("lastPoint2: [", lastPoint[0], ",", lastPoint[1], "]")
            for i in range(0,5):
                path.L(lastPoint[0], lastPoint[1] - 320)
                lastPoint[1] -= 320
                path.L(lastPoint[0], lastPoint[1] + 320)
...
if __name__=="__main__":
    SimpleMaker.GenerateSimple("stretch_black", -8, 10)
